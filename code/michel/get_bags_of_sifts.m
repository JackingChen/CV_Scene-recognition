...
%}
use_method = 'KD'; %'KD' or 'dist'

load('vocab.mat')
vocab_size = size(vocab, 2);
image_feats = zeros([length(image_paths),vocab_size]);
dsift_step = 5;
if strcmp(use_method,'KD'); kdtree = vl_kdtreebuild(vocab');end
    
for i=1:length(image_paths);
    % read (avoid RGB problem) & SIFT
    ima = imread(image_paths{i});
    if 1 < size(ima,3); ima=rgb2gray(ima); end
    [~, features] = vl_dsift( single(ima) , 'step',dsift_step );%, 'size',dsift_size
    switch use_method
        case 'dist'
            D = vl_alldist2(double(features),vocab');   % distance
            [~,I] = min(D,[],2);                        % find the closest
        case 'KD'
            I = vl_kdtreequery(kdtree, vocab', double(features));
    end
    % count & normalize
    I=double(I);
    [count,name]=hist(I,min(I):max(I));
    feat = count/sum(count);
    image_feats(i,name) = feat; 
end
