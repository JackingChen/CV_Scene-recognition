...
% initialize
LAMBDA = 1;%0.00001;
W = cell(num_categories,1);
B = cell(num_categories,1);

% categories & train the model
for i = 1:num_categories
    matching_indices = strcmp(categories(i),train_labels);  % [ 0 or 1 ]
    labels = 2*matching_indices-1;                          % [ (-1) => 2*0-1 or (1) => 2*1-1 ]
    [W{i},B{i}] = vl_svmtrain(train_image_feats.', labels, LAMBDA);
end

% predict test-data in each model
predict_score = zeros(size(test_image_feats,1), num_categories);
for j = 1:num_categories
    predict_score(:, j) = test_image_feats*W{j} + B{j};
end
% find the most possible (high score)
[~, I_michel] = max(predict_score,[],2);
predicted_categories = categories(I_michel);
